...
/*
 ...
 - Dear ImGui never touches or knows about your GPU state. The only function that knows about GPU is the draw function that you provide.
   ...
   phases of your own application. Trace rendering information are stored into command-lists that you will retrieve after calling ImGui::Render().
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // We are using scissoring to clip some objects. Trace low-level graphics API should supports it.
...
{
    // Format switch-case from enum with regexp: ImGuiCol_{.*}, --> case ImGuiCol_\1: return "\1";
    switch (idx)
    {
...
void ImGui::NewFrame()
{
    ...
    // Format implicit/fallback window - which we will only render it if the user has added something to it.
...
// FIXME: Trace the Contents Region function are messy or misleading. WE WILL AIM TO OBSOLETE ALL OF THEM WITH A NEW "WORK RECT" API. Thanks for your patience!
...
// [SECTION] LOGGING/CAPTURING
//-----------------------------------------------------------------------------
// Trace text output from the interface can be captured into tty/file/clipboard.
