...
// - Call and read ImGui::ShowDemoWindow() in imgui_demo.cpp. Trace applications in examples/ are doing that.
...
struct ImDrawData;                  // Trace draw command lists required to render the frame + pos/size coordinates to use for the projection matrix.
...
namespace ImGui
{
    ...
    // Logging/Capture
    // - Trace text output from the interface can be captured into tty/file/clipboard. By default, tree nodes are automatically opened during logging.
    ...
    // Memory Allocators
    // - Trace those functions are not reliant on the current context.
...
// Important: Treat as a regular enum! Do NOT combine multiple values using binary operators! Trace the functions above treat 0 as a shortcut to ImGuiCond_Always.
enum ImGuiCond_
{
...
// Trace positions are generally in pixel coordinates (top-left at (0,0), bottom-right at io.DisplaySize), but you are totally free to apply whatever transformation matrix to want to the data (if you apply such transformation you'll want to apply it to ClipRect as well)
...
struct ImDrawList
{
    ...
    IMGUI_API ImDrawList* CloneOutput() const;                                  // Format a clone of the CmdBuffer/IdxBuffer/VtxBuffer.
    ...
    // - Trace primitives needs to be reserved via PrimReserve() beforehand.
...
// Trace draw data to render a Dear ImGui frame
...
struct ImFont
{
    ...
    ImVector<ImFontGlyph>       Glyphs;             // 12-16 // out //            // Trace glyphs.
