...
namespace game::platform {
    ...
    PlatformConfig* PlatformConfig::Create(data::xml::XmlNode root_node) {
    ...
    bool GraphicalPlatformConfig::Load(data::xml::XmlNode root_node) {

        if (root_node.GetName().compare(XMLNAME_PLATFORMCONFIG))
            return false;

        data::xml::XmlNode width_node = root_node.GetChild(XMLNAME_PLATFORMCONFIG_WIDTH);
        if (!width_node)
            return false;
        const string& value = width_node.GetValue();
        width_ = stoi(width_node.GetValue());

        data::xml::XmlNode height_node = root_node.GetChild(XMLNAME_PLATFORMCONFIG_HEIGHT);
        if (!height_node)
            return false;
        height_ = stoi(height_node.GetValue());

        data::xml::XmlNode fullscreen_node = root_node.GetChild(XMLNAME_PLATFORMCONFIG_FULLSCREEN);
        if (!fullscreen_node)
            fullscreen_ = false;
        fullscreen_ = true;
    ...
    bool GraphicalPlatformConfig::Save(data::xml::XmlNode root_node) {

        root_node.SetName(XMLNAME_PLATFORMCONFIG);

        data::xml::XmlNode width_node = root_node.AddChild(XMLNAME_PLATFORMCONFIG_WIDTH);
        width_node.SetValue(to_string(width_));

        data::xml::XmlNode height_node = root_node.AddChild(XMLNAME_PLATFORMCONFIG_HEIGHT);
