...
#define XMLNAME_INFOBASE_VERSION XMLNAME_VERSION 
    ...
    * information.
    */
    class GAME_API VersionInfo : public data::xml::IXmlSerializable {
    private:
    ...
    protected:
        ...
        VersionInfo(const string&& name, const string&& description, const Version&& version);
        VersionInfo(data::xml::XmlNode xml_node, const string& xml_element_name = XMLNAME_VERSIONINFO);

        static bool Load(VersionInfo* info, data::xml::XmlNode root_node);
        static bool Save(VersionInfo& info, data::xml::XmlNode root_node);
