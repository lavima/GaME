...
namespace game {
    ...
    VersionInfo::VersionInfo(data::xml::XmlNode xml_node, const string& xml_element_name) {
    ...
    bool VersionInfo::Load(VersionInfo* info, data::xml::XmlNode root_node) {

        if (root_node.GetName()!=info->xml_element_name_)
            return false;

        data::xml::XmlNode name_node = root_node.GetChild(XMLNAME_INFOBASE_NAME);
        if (!name_node)
            return false;
        info->name_ = string(name_node.GetValue());

        data::xml::XmlNode description_node = root_node.GetChild(XMLNAME_INFOBASE_DESCRIPTION);
    ...
    bool VersionInfo::Save(VersionInfo& info, data::xml::XmlNode root_node) {

        if (info.xml_element_name_!="")
            root_node.SetName(info.xml_element_name_);

        data::xml::XmlNode name_node = root_node.AddChild(XMLNAME_INFOBASE_NAME);
    ...
    bool VersionInfo::Load(data::xml::XmlNode root_node) { return VersionInfo::Load(this, root_node); }
    bool VersionInfo::Save(data::xml::XmlNode root_node) { return VersionInfo::Save(*this, root_node); }
